...
main :: IO ()
main = interact $ show . findCycleLength . parse . lines . tabToNewline

findCycleLength :: Memory -> Int
findCycleLength m =
    let (outer,targetM) = findCycles m
        prefix = findConfig targetM m
    in outer - prefix

findConfig :: Memory -> Memory -> Int
findConfig tm m =
    let findConfig' i m =
            let m' = reallocate m
            in if m' == tm
            then i + 1
            else findConfig' (i+1) m'
    in findConfig' 0 m

findCycles :: Memory -> (Int, Memory)
findCycles m =
